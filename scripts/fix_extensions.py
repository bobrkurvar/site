...
from domain import Collection, Image
...
def fix_fs():
    if not TARGET_DIR.exists():
        log.error(
            f"Директория {TARGET_DIR} не найдена! Проверь volumes в docker-compose."
        )
        return
        ...
        if file_path.is_file() and not file_path.suffix:
            try:
                ...
                with Image.open(file_path) as img:
                    ...
                    log.info(
                        f"Успешно переименован: {file_path.name} -> {new_path.name}"
                    )
                    fixed_count += 1
                ...
                log.warning(
                    f"Файл {file_path.name} не является валидным изображением. Пропуск."
                )
                skipped_count += 1
    ...
    log.info(
        f"Сканирование завершено. Переименовано: {fixed_count}, пропущено: {skipped_count}"
    )
        ...
        for item in items:
            ...
            if matching_files:
                ...
                if real_file != new_file_path:
                    ...
                    log.info(
                        f"Переименован на диске: {real_file.name} -> {new_filename}"
                    )
                new_db_path = (path_obj.parent / new_filename).as_posix()
                log.info(f"Переименован: {real_file.name} -> {new_filename}")
                await manager.update(
                    model, {"image_path": image_path}, image_path=new_db_path
                )

            else:
                log.warning(
                    f"Файл для записи {image_path} не найден ни с одним расширением"
                )
