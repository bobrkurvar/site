...
from infra.security import create_token_jti, create_token_family_id
from domain import Admin, CredentialsValidateError, UserLoginNotFoundError, RefreshTokenFamilyExpiredError, RefreshTokenReusedCompromisedError
...
async def test_get_tokens_from_login_success(crud, redis):
    ...
    tokens = await create_tokens_from_login(manager=crud, redis=redis, username=username, password=password, verify=verify)
...
async def test_get_tokens_from_login_wrong_password(crud, redis):
    ...
    with pytest.raises(CredentialsValidateError):
        await create_tokens_from_login(manager=crud, redis=redis, username=username, password=wrong_password, verify=verify)
...
async def test_get_tokens_from_login_wrong_username(crud, redis):
    ...
    with pytest.raises(UserLoginNotFoundError):
        await create_tokens_from_login(manager=crud, redis=redis, username=wrong_username, password=password, verify=verify)
...
async def test_get_new_tokens_from_refresh_fail_when_token_already_used(redis):
    data = {"user": "user", "id": 1}
    family_id, jti1, jti2 = create_token_family_id(), create_token_jti(), create_token_jti()
